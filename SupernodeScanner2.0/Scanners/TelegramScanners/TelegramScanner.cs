...
namespace SuperNodeScanner
{
    internal class TelegramScanner
    {
        ...
        private SummaryCreator SummaryCreator { get; set; }
        public TelegramScanner()
        {
            SummaryCreator = new SummaryCreator(new Connection());
        }
                            ...
                            case "/harvestingSpace":
                            {
                                ...
                                snodeClient.BeginGetSupernodes(ar =>
                                {
                                    ...
                                    for (var index = r.Next(minValue: 0, maxValue: 320); index < 400; index++)
                                    {
                                        ...
                                        {
                                            if (ar2.Content.NumUnlocked < ar2.Content.MaxUnlocked) freeNodes.Add(item: node.ip);
